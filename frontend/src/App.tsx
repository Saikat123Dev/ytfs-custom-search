...
function App() {
  ...
  useEffect(() => {
    if (iframeRef.current && videoId && currentTime > 0) {
      const newSrc = `https://www.youtube.com/embed/${videoId}?start=${currentTime}&autoplay=1`
  ...
  <label className="relative inline-flex items-center cursor-pointer">
    ...
    <div className="w-11 h-6 bg-gray-500 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-blue-500">
        ...
        <button
          ...
          className="w-full bg-red-500 hover:bg-red-600 disabled:bg-gray-400 disabled:cursor-not-allowed text-white py-3 px-4 rounded-xl transition-colors flex items-center justify-center gap-2 shadow-sm font-medium"
        >
          {isLoading ? (
            <>
     ...
     <div className='space-y-3 max-h-80 overflow-y-auto pr-2'>
      ...
      {/* Video Preview */}
      {isValidLink && showPreview && videoId && (
        <div className="mb-6">
          <div className="bg-white rounded-xl shadow-sm overflow-hidden">
            <div className="aspect-video bg-gray-100">
              <iframe
                ref={iframeRef}
                src={`https://www.youtube.com/embed/${videoId}${currentTime > 0 ? `?start=${currentTime}&autoplay=1` : ''}`}
            ...
            <div className="p-4">
              <p className="text-xs text-gray-600 break-all mb-2">
                Video ID: <span className="font-mono bg-gray-100 px-2 py-1 rounded">{videoId}</span>
              </p>
              {currentTime > 0 && (
      ...
      {isValidLink && (
        <div className="mb-6">
          <button
            onClick={() => openInYouTube(`https://www.youtube.com/watch?v=${videoId}${currentTime > 0 ? `&t=${currentTime}` : ''}`)}
      ...
      <div className="text-center">
        <p className="text-xs text-gray-500">
          Supports youtube.com and embed links
        </p>
      </div>
    </div>
  )
